...
function getUpdateHoverContent(updateState: StateType): MarkdownString {
	...
	switch (updateState) {
		case StateType.AvailableForDownload:
			hoverContent.appendMarkdown(localize('chat.modelPicker.downloadUpdateHover', "This model requires a newer version of Stokd Code. [Download Update](command:update.downloadUpdate) to access it."));
			break;
		case StateType.Downloaded:
		case StateType.Ready:
			hoverContent.appendMarkdown(localize('chat.modelPicker.restartUpdateHover', "This model requires a newer version of Stokd Code. [Restart to Update](command:update.restartToUpdate) to access it."));
			break;
		default:
			hoverContent.appendMarkdown(localize('chat.modelPicker.checkUpdateHover', "This model requires a newer version of Stokd Code. [Update Stokd Code](command:update.checkForUpdate) to access it."));
			break;
	}
	return hoverContent;
}
...
function createUnavailableModelItem(
	...
	if (reason === 'upgrade') {
		...
		description = localize('chat.modelPicker.updateDescription', "Update Stokd Code");
	} else {
